...
public:
                          ...
                          const std::vector<Eigen::Vector3d> &route)
    {
        ...
        if (traj.getPieceNum() > 0)
        {
            ...
            RCLCPP_INFO(node->get_logger(), "Visualizing trajectory with %d points", (int)trajMarker.points.size());
    ...
    inline void visualizePolytope(const std::vector<Eigen::MatrixX4d> &hPolys)
    {
        ...
        RCLCPP_INFO(node->get_logger(), "Visualizing mesh with %d points", ptnum);
                                ...
                                const double &radius)
    {
        ...
        point.z = center(2);
            ...
            //RCLCPP_INFO(node->get_logger(), "Visualizing sphere with center (%.2f, %.2f, %.2f) and radius %.2f", center(0), center(1), center(2), radius);
    ...
    inline void visualizeStartGoal(const Eigen::Vector3d &center,
                                   ...
                                   const int sg)
    {;
         RCLCPP_INFO(node->get_logger(), "Visualizing start/goal point with position (%.2f, %.2f, %.2f)", center(0), center(1), center(2));
        visualization_msgs::msg::Marker sphereMarkers, sphereDeleter;

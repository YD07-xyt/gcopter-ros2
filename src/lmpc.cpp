#include "../include/controller/lmpc.hpp"

Mpc::Mpc(int mpc_window, double dt) {
    mpc_window_ = mpc_window;
    dt_ = dt;
...
void Mpc::setWeightMatrices(Eigen::DiagonalMatrix<double, 2> &Q, Eigen::DiagonalMatrix<double, 2> R) {
    //  增大 R 会惩罚大幅速度变化，导致机器人运动更平滑但跟踪可能偏慢；

    //  减小 R  允许更快的速度指令，响应更迅速；

    // 增大 Q 会增强对位置误差的惩罚，使机器人更快接近参考轨迹。
